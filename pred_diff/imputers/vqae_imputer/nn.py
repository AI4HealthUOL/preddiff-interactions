...
def get_name(layer_name, counters): #[nn.wnconv2d, nn.wndense, nn.conv2d, nn.gated_conv2d, nn.gated_deconv2d]
...
def conv2d(x, num_filters, filter_size=[3,3], stride=[1,1], pad='SAME', nonlinearity=None, counters={}, ema=None, **kwargs):
    """ convolutional layer """
    name, counters = get_name('conv2d', counters)
    xs = int_shape(x)
...
def deconv2d(x, num_filters, filter_size=[3,3], stride=[1,1], pad='SAME', nonlinearity=None, counters={}, ema=None, **kwargs):
    ...
    name, counters = get_name('deconv2d', counters)
    xs = int_shape(x)
    # See https://arxiv.org/abs/1502.03167v3.
    input_feature_size = filter_size[0]*filter_size[1]*xs[3]
    stddev = 1. / math.sqrt(input_feature_size)
...
def wndense(x, num_units, nonlinearity=None, counters={}, ema=None, **kwargs):
    ...
    name, counters = get_name('wndense', counters)
    with tf.variable_scope(name):
...
def wnconv2d(x, num_filters, filter_size=[3,3], stride=[1,1], rate=1, pad='SAME', nonlinearity=None, counters={}, ema=None, **kwargs):
    ...
    name, counters = get_name('wnconv2d', counters)
    with tf.variable_scope(name):
...
#@add_arg_scope
def nin(x, num_units, counters={}, ema=None, **kwargs):
    """ a network in network layer """
    s = int_shape(x)
    x = tf.reshape(x, [np.prod(s[:-1]),s[-1]])
    x, counters = wndense(x, num_units, counters=counters, ema=ema, **kwargs)
    return tf.reshape(x, s[:-1]+[num_units]), counters
...
def gated_conv2d(x, num_filters, filter_size=[3,3], stride=[1,1], rate=1, pad='SAME', nonlinearity=tf.nn.elu, counters={}, **kwargs):
    ...
    name, counters = get_name('gated_conv2d', counters)
    xs = int_shape(x)
    # See https://arxiv.org/abs/1502.03167v3.
    input_feature_size = filter_size[0]*filter_size[1]*xs[3]
    stddev = 1. / math.sqrt(input_feature_size)
...
def gated_deconv2d(x, num_filters, filter_size=[3,3], stride=[1,1], pad='SAME', nonlinearity=tf.nn.elu, counters={}, **kwargs):
    ...
    name, counters = get_name('gated_deconv2d', counters)
...
def snconv2d(x, num_filters, filter_size=[5,5], stride=[2,2], pad='SAME', nonlinearity=None, counters={}, **kwargs):
    ...
    name, counters = get_name('snconv2d', counters)
    xs = int_shape(x)
    with tf.variable_scope(name):
        W = tf.get_variable('W', shape=filter_size+[int(x.get_shape()[-1]),num_filters], dtype=tf.float32,
...
def vector_quantize(inputs, embedding_dim=64, num_embeddings=512, commitment_cost=0.25, decay=0.99, epsilon=1e-5, is_training=False, counters={}, **kwargs):
    """ vector quantizer """
    name, counters = get_name('vector_quantize', counters)
    with tf.variable_scope(name):
        ...
        return quantized, loss, encoding_indices, w.read_value(), counters

#@add_arg_scope
def resnet(x, num_res_channel=64, nonlinearity=tf.nn.elu, conv=conv2d, counters={}, ema=None, **kwargs):
    xs = int_shape(x)
    num_filters = xs[-1]

    c1, counters = conv(nonlinearity(x), num_res_channel, counters=counters, ema=ema)
    c2, counters = conv(nonlinearity(c1), num_filters, filter_size=[1,1], counters=counters, ema=ema)

    return x + c2, counters

#@add_arg_scope
def cond_resnet(x, nonlinearity=concat_elu, conv=wnconv2d, rate=1, counters={}, ema=None, **kwargs):
    xs = int_shape(x)
    num_filters = xs[-1]

    c1, counters = conv(nonlinearity(x), num_filters * 2, rate=rate, counters=counters, ema=ema)
...
#@add_arg_scope
def out_resnet(x, nonlinearity=tf.nn.elu, conv=nin, counters={}, ema=None, **kwargs):
    xs = int_shape(x)
    num_filters = xs[-1]

    c1, counters = conv(nonlinearity(x), num_filters, counters=counters, ema=ema)

    return x + c1, counters

#@add_arg_scope
def gated_resnet(x, a=None, h=None, num_res_filters=128, nonlinearity=concat_elu, conv=wnconv2d, rate=1, dropout_p=0., causal_attention=False, num_head=8, counters={}, ema=None, **kwargs):
    xs = int_shape(x)
    num_filters = xs[-1]

    c1, counters = conv(nonlinearity(x), num_res_filters, rate=rate, counters=counters, ema=ema)
    if a is not None: # add short-cut connection if auxiliary input 'a' is given
        tmp, counters = nin(nonlinearity(a), num_res_filters, counters=counters, ema=ema)
        c1 += tmp
    c1 = nonlinearity(c1)
    
    c2, counters = conv(c1, num_filters * 2, rate=1, counters=counters, ema=ema)
    if h is not None: # add condition h if included: conditional generation
        tmp, counters = nin(nonlinearity(h), num_filters * 2, counters=counters, ema=ema)
        c2 += tmp
    ...
    if causal_attention:
        ...
        for head_rep in range(num_head):
            query, counters = nin(c3, num_filters//8, counters=counters, ema=ema)
            key, counters = nin(c3, num_filters//8, counters=counters, ema=ema)
            value, counters = nin(c3, num_filters//2, counters=counters, ema=ema)
        ...
        multihead, counters = nin(multihead, num_filters, counters=counters, ema=ema)
        c3 = c3 + multihead
    
    if dropout_p > 0:
        c3 = tf.nn.dropout(c3, keep_prob=1. - dropout_p)

    return x + c3, counters
